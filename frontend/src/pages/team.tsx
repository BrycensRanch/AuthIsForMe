									...
									<div className="my-1 w-full px-1 md:w-1/2 lg:my-4 lg:w-1/3 lg:px-4">
										<div className="w-full max-w-sm rounded-lg border border-gray-200 bg-white shadow dark:border-gray-700 dark:bg-gray-800">
											<div className="flex flex-col items-center pb-10">
												<img
													className="my-3 h-24 w-24 rounded-full shadow-lg"
													src={member.profilePictureURL}
													alt={`${member.name}'s GitHub Profile Picture`}
													id={`${member.id}-img`}
												/>
												<h5 className="mb-1 text-xl font-medium text-gray-900 dark:text-white" id={member.id}>
													{member.name}
												</h5>
												<span
													className="text-sm text-gray-500 dark:text-gray-400"
													id={`${member.id}-contributionLevel`}
												>
													{member.contributionLevel}
												</span>
												<p id={`${member.id}-bio`} className="text-center font-satoshi font-bold italic">
													"{member.bio}"
												</p>
													...
													<>
														...
														{member.contributions?.map((contribution: any) => (
															<span
																...
																key={`${member.id}-contribution-${contribution}`}
															>
																{contribution}
															</span>
														))}
												...
												<div className="mt-4 flex space-x-3 md:mt-6">
													...
													>
														View {member.URL?.includes('github.com') ? 'GitHub' : 'Site'}
													</a>
													{member.email ? (
														<Link
															href={`mailto:${member.email}?subject=Let's%20chat!&body=Hey%2C%20I%20saw%20you%20in%20${AppConfig.site_name}'s%20Team%20page.%20Wanted%20to%20contact%20you%20regarding%2C%20x%2C%20y%2C%20and%20finally%2C%20z.`}
