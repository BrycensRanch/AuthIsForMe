			...
			await Promise.all(
				[...Object.keys(dependencies), ...Object.keys(devDependencies)].map(async dep => {
					...
					try {
						...
						const normalizedRepoURL = new URL(npmPackage.repository?.url.replace(/\.git$/, '').replace('git+', '') || `https://github.com/npm/${dep}`);
		...
		<Main meta={<Meta title="Thanks" description={''} />}>
			<main className="mx-auto max-w-7xl py-6 px-4">
				...
				<p className="ml-2 font-semibold">
					...
					page. Due to the amount of resources this page already uses, we cannot show a direct copy of the licenses. You
