...
const LogoIcon = '/assets/images/logo/logo-icon.svg';
			...
			<div className="flex grow items-center justify-between p-4 shadow-2 md:px-6 2xl:px-11">
				<div className="flex items-center gap-2 sm:gap-4 lg:hidden">
					...
					<button
						...
						aria-expanded={properties.sidebarOpen}
						onClick={event => {
							event.stopPropagation();
							properties.setSidebarOpen(!properties.sidebarOpen);
						}}
							...
							<span className="du-block absolute right-0 size-full">
								...
								<span
									className={`relative left-0 top-0 my-1 block h-0.5 w-0 rounded-sm bg-black delay-150 duration-200 ease-in-out dark:bg-white ${
										!properties.sidebarOpen && 'delay-400 !w-full'
									}`}
								></span>
								<span
									className={`relative left-0 top-0 my-1 block h-0.5 w-0 rounded-sm bg-black delay-200 duration-200 ease-in-out dark:bg-white ${
										!properties.sidebarOpen && '!w-full delay-500'
									}`}
								></span>
							</span>
							<span className="absolute right-0 size-full rotate-45">
								...
								<span
									className={`delay-400 absolute left-0 top-2.5 block h-0.5 w-full rounded-sm bg-black duration-200 ease-in-out dark:bg-white ${
										!properties.sidebarOpen && '!h-0 !delay-200'
									}`}
								></span>
							</span>
						</span>
					...
					<Link className="block shrink-0 lg:hidden" href="/" legacyBehavior>
